...
export function SectionHeading({ eyebrow, title, description, align = "center" }: Props) {
  return (
    <div className={align === "center" ? "text-center max-w-xl mx-auto" : ""}>
      ...
      <h2 className="text-pretty text-2xl sm:text-3xl font-semibold tracking-tight">{title}</h2>
      {description ? <p className="mt-3 text-sm text-muted-foreground leading-relaxed">{description}</p> : null}
    </div>
  )
}
